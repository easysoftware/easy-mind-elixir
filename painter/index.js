...
function generateSvgDom(instance) {
  let svgContent = ''

  //main node
  const mainNode = $d.querySelectorAll('root')[0]
  let rect = mainNode.getBoundingClientRect()
  let top = mainNode.offsetTop
  let bottom = top + rect.height
  let left = mainNode.offsetLeft
  ...
  svgContent += PrimaryToSvg(mainNode)
  ...
  for (let i = 0; i < primaryNodes.length; i++) {
    ...
    top = primaryNode.offsetTop
    bottom = top + rect.height
    left = primaryNode.offsetLeft
...
function PrimaryToSvg(primaryNode, offset = 0) {
  ...
  let primaryNodeOffsetX = primaryNode.offsetLeft - maxLeft + offset
  let svg = ''
...
export let exportSvg = function(instance, fileName, download = true) {
  ...
  let svgFile = generateSvgDom(instance)
  if (download) {
...
export const getJSONDepth = ({ children }) => 1 + (children && children.length ? Math.max(...children.map(getJSONDepth)) : 0)
